...
class RunningScore():
    ...
    def update_rs(self, player, editFlag):
        ...
        if editFlag and len(self.rScore) > 1:
            del(self.rScore[-1])
            for rsList in self.rsLists:
                del(rsList[-1])
        # else trap not to delete running score and crash, NEED TO FIX ****
        elif editFlag and len(self.rScore) <= 1:
            pass
        else:
            if player.team == 'Home':
